...
Mat l_Region_of_Interest_crop(Mat image)
{
    Mat img_roi_crop;	
    int row_crop = 340;
    Rect rect(0,row_crop,image.cols,image.rows-row_crop);	 
...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
        ...
        Mat l_image;
        lcap >> l_image;
        Mat l_roi_img = l_Region_of_Interest_crop(l_image);
        imshow("l_roi_img",l_roi_img);
        Mat src;
        image.copyTo(src);

        Mat roi_img = Region_of_Interest_crop(image);

        Mat image_gray, thresh_img;
        cvtColor(roi_img, image_gray, COLOR_RGB2GRAY);
        
        threshold(image_gray, thresh_img, 135, 255, THRESH_BINARY);
        imshow("thresh_img",thresh_img);
        Mat canny_edge = Canny_Edge_Detection(thresh_img);
        ...
        sprintf(filepath,"/home/fc/catkin_ws/src/gm/opencv_line_detect_cpp/src/data/%d/%d",LANE,i);
        strftime (filename,80,"%m_%e_%T.jpg",timeinfo);
        strcat(filepath, filename);
        imwrite(filepath, l_roi_img);
